...
- An unwritable data directory is now reported as exactly that, naming the directory, the effective uid and gid, and what to change. It previously surfaced as `sqlite3.OperationalError: attempt to write a readonly database` raised from a `PRAGMA`, which named neither the directory nor the user. The message also calls out the case that produced it in practice: dropping all capabilities strips root's ability to bypass file permissions, so a container running as root is still subject to the directory mode. An unwritable database file is reported the same way.
